...
const FIELDS: Field[] = [
  /*
   * **매일 쌓는 것을 맨 위에 둔다.** 아이가 가장 자주 만나는 금액이고, 레벨업은
   * 몇 달에 한 번이다. 자주 걸리는 것이 위에 있어야 부모가 먼저 정한다.
   */
  {
    key: 'dailyDone',
    label: '하루치를 다 마쳤을 때',
   ...
   * **「한 달 개근」 은 뺐다.**
   *
   * 하루도 안 빠져야 받는 것이라, 중순에 한 번 빠지면 남은 보름을 버틸 이유가
   * 사라졌다. 매일 쌓는 쪽(하루치)과 달 말 공로금으로 갈음했다 — 한 번 빠져도
   * 그날치만 없고, 스무닷새를 넘기면 얹어 줄 수 있다.
   *
   * 예전에 금액을 정해 둔 프로필이 있어도 `awardRates()` 가 0 으로 눌러 둔다.
   */

  /*
   * **「아이가 더 요구할 수 있는 금액」 은 뺐다.**
   *
   * 아이가 기본 금액 위에 한 칸 더 얹어 신청할 수 있던 칸이다. 정하는 자리가
   * 여기 있으면 부모는 그것까지 정해야 하고, 아이 쪽에는 "더 달라고 해 볼까"
   * 를 누르는 자리가 생긴다. 요청권은 '레벨을 끝냈다' 는 사실 하나로 값이
   * 정해지는 편이 서로 덜 피곤하다.
   *
   * 규칙 자체(awards.ts 의 bonus·bonusCap)는 남겨 둔다 — 이미 신청해 둔 것에
   * 그 값이 적혀 있어서, 지우면 지난 기록의 금액이 달라진다.
   */
];

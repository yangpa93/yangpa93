...
async function seed(page, label) {
  ...
  /*
   * **누른 뒤를 기다리지 않는다**(`noWaitAfter`).
   *
   * 데모 단추는 `location.href = '/'` 로 앱을 새로 띄운다. 기본값이면
   * playwright 가 그 이동이 끝날 때까지 기다리는데, 번들이 커지면서 서른 초를
   * 넘겨 시험이 통째로 죽는 일이 생겼다 — 데모를 심다가 죽으니 무엇이
   * 잘못됐는지도 안 보였다.
   *
   * 눌러 두고 아래에서 시간을 세어 기다린다. 앱이 뜨는 것은 그다음 `go()` 가
   * 화면을 보고 확인한다.
   */
  await page
...
/*
 * **아이 고르는 화면에는 부르기도 설정도 없다.**
 *
 * 부르는 자리를 두 번 옮겼다. 아이 개별 설정 안(설정 네 단계 안쪽) → 이 화면
 * → 아이별 보고서 안. 여기 두었을 때는 셋이 나란히 늘어서서, 아이를 고르는
 * 화면에서 또 아이를 골라야 했다. 지금은 아이 하나를 골라 들어간 보고서 안에
 * 그 아이 것만 있다.
 */
ok(
  '아이 고르는 화면에는 부르기가 없다',
  !(await has(page, '공부하세요', 1500)),
  '아직 남아 있다',
);
ok(
  '아이 고르는 화면에는 설정 링크가 없다',
  !(await has(page, '아이별 설정 하기', 1500)),
  '아직 남아 있다',
);
...
/*
 * **상 주기와 부르기는 그 아이 보고서 안에 있다.**
 *
 * 기록을 보다가 하고 싶어지는 일이라 그 자리에 둔다. "오늘 다 했네" 를 보면
 * 상을 주고 "아직 안 했네" 를 보면 부른다. 승인 화면은 따로 있었지만 부모
 * 홈에서 그리로 가는 길이 없어서, 알림만 오고 승인할 자리를 못 찾는다는 말을
 * 들었다.
 *
 * 「직접 쓰기」 는 빈 칸의 안내 글자(placeholder)다. `has` 는 텍스트 노드를
 * 보므로 안 잡힌다 — 이 파일 앞머리에 적어 둔 그 함정에 그대로 걸렸다.
 */
await go(page, `/calendar?name=${encodeURIComponent('수빈')}`);
ok('아이별 보고서에 올린 상이 있다', await has(page, '올린 상', 3000));
ok('상 금액이 적혀 있다', await has(page, '500원', 3000));
ok('주기로 하기가 있다', await has(page, '주기로 하기', 3000));
ok('같은 화면에 공부하세요가 있다', await has(page, '공부하세요', 3000));
ok('보낼 말이 바로 열려 있다', await hasField(page, '직접 쓰기', 3000));
ok('그 아이에게만 보내는 단추가 있다', await has(page, '수빈에게 보내기', 3000));

/* 설정 화면에는 부르기가 없어야 한다 — 값을 고치는 자리와 섞이지 않게. */
await go(page, `/parent-child-one?name=${encodeURIComponent('수빈')}`);
ok('아이 설정에는 부르기가 없다', !(await has(page, '공부하세요', 1500)), '아직 남아 있다');
/*
 * **갈래와 하루 분량도 여기서 정한다.**
 *

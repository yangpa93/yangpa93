# 인수인계 — 영한 대조 (A·B 모두 끝남)

이 문서는 새 세션에 넘기는 인수인계다. 지금까지 무엇을 했고, 무엇이
남았고, 무엇을 건드리면 안 되는지 적어 둔다.

할 일은 두 가지였다. **둘 다 끝났다(2026-07-29).**

- **A. 영한 사전으로 '뜻' 대조** — 끝. 3,285개 전수 대조. 뜻 오류 0건.
- **B. 품사·유의어 표시 고치기** — 끝.

아래는 무엇을 어떻게 했고 무엇을 건드리면 안 되는지 남기는 기록이다.
다시 하지 말고, 이어서 할 일이 생기면 맨 아래 「남은 것」을 본다.

## A·B 를 어떻게 끝냈나

### A — 위키낱말사전 덤프로 대조

kaikki.org 의 영어 덤프(JSONL, 3.19GB)를 받아 우리 어휘만 걸러 쓴다.

```bash
npm run reference ref.json        # 참고 자료 만들기 (덤프를 흘려보내며 거른다)
npm run verify:meaning ref.json          # 전체
npm run verify:meaning ref.json m1-1     # 한 레벨만
LIMIT=2000 npm run verify:meaning ref.json   # 다 보기
```

**HANDOFF 에 적힌 kaikki 낱말별 URL 은 404 다.** 그 경로는 없어졌고 전체
덤프만 있다. 그리고 3GB 를 한 번에 요청하면 프록시가 응답을 통째로 버퍼에
담으려다 멈춰 선다(2분에 1MB). 범위 요청으로 100MB씩 끊어 받으면 6MB/s 다.
`fetch-reference.mjs` 가 그렇게 되어 있다.

거르는 조건을 손볼 때는 덤프를 한 번 받아 두고 `VOCAB_DUMP=경로` 로 읽는다.
조건이 틀리면 3GB 를 다시 받게 되는데, 실제로 한 번 그랬다.

**뜻 대조는 유의어가 아니라 한국어 번역으로 한다.** 유의어로 맞대면
ocean/sea, prison/jail 이 다 걸린다 — 위키낱말사전이 뜻마다 유의어를 잘 달지
않기 때문이지 우리가 틀려서가 아니다. 덤프에는 뜻마다 한국어 번역이 붙어
있다(ocean → [대양, 바다]). 한국어를 한국어와 맞대는 쪽이 훨씬 곧다.

헛걸림을 걷어낸 자리들 — 다시 밟지 말 것.

| 무엇 | 왜 | 건수 |
|---|---|---|
| 유의어 → 한국어 번역 | 위 | 264 → |
| 품사를 맞춰서 본다 | brush 를 동사로 실었는데 명사 번역과 맞대고 있었다 | 313 → 260 |
| 받침을 떼고 견준다 | ㅂ·ㅎ 불규칙. '어두운/어둡다', '파란/파랗다' | 260 → 232 |
| 굴절형을 자르지 않는다 | be 는 39개인데 12개에서 끊어 is 가 빠졌다 | 예문 6 → 0 |
| 숙어는 갈라진다 | "take the weather into account" | 위와 같음 |
| adj.↔det, int.↔particle | 학교문법과 위키낱말사전이 다르게 가른다 | 품사 35 → 1 |

**결과 — 뜻이 틀린 단어는 한 개도 없었다.** 남은 232건은 전부 위키낱말사전
쪽이 다른 뜻만 담아 둔 자리다(pool→포켓볼, corn→티눈). 사전이 틀린 것도
있다(remedy→요약, lend→빌리다). 더 조여도 얻을 것이 없다.

실제로 고친 것은 둘뿐이다.

- `okey` 표제어 제거 — 일곱 줄 위에 `okay` 가 같은 뜻으로 있었고, `okey` 는
  표준 철자가 아닌데 예문이 "Everything looks okey now." 였다. `plan.ts` 에서도
  뺐다(안 빼면 m2-1 이 136/137 이 되어 '앞 레벨부터 순서대로' 규칙이 깨진다).
  h3-3·h3-4 가 이미 136개라 136 자체는 문제가 아니다.
- `yes` `yeah` `yep` `nope` 품사를 `adv.` → `int.` 로.

`million` 은 `n.` 인데 사전은 `num` 으로만 본다. 학교문법은 명사로도 가르치니
그대로 두기로 했다. **다시 지적하지 말 것.**

### B — 화면 표시

- `posLabel()` — `'v., adj.'` → `'동사 · 형용사'`. 데이터의 `p` 는 그대로 둔다.
  모르는 값이 섞이면 원래 문자열을 돌려준다(빈칸이 되면 안 된다).
- 유의어 문구 — `= firm` 을 없앴다. 사람과 정한 문구는 이것이다.

  ```
  "단단한" 이라는 뜻일 때 이렇게 바꿔 쓸 수 있어요 : firm, hard
  ```

  **유의어를 문장 안에 넣지 않은 것은 조사 때문이다.** `firm 로` 는 틀렸다 —
  '펌'이라 `으로`가 맞고 `hard` 는 '하드'라 `로`가 맞다. 받침이 붙는지는 영어
  낱말을 한글로 어떻게 읽느냐에 달렸고(look→룩, put→풋) 유의어 목록 끝에 오는
  낱말이 1,600가지다. 규칙으로 고르면 반드시 틀린 자리가 생긴다.
- 품사는 **단어 카드에서만** 칩으로. 단어장·오답 노트는 한 줄에 여러 단어가
  늘어서므로 회색 작은 글씨 그대로.
- 유의어도 눌러서 들을 수 있게 했다. 세 화면이 같은 줄을 각자 그리고 있어서
  `src/components/SynonymLine.tsx` 하나로 묶었다.

---

# A. 영한 사전으로 '뜻' 대조 — **끝남**

아래는 이 일을 시작할 때 적어 둔 계획이다. 실제로 어떻게 됐는지는
맨 위 「A·B 를 어떻게 끝냈나」를 본다. 계획과 다른 곳이 몇 군데 있다.
...
중복 예문 134건은 원래 있던 경고다.

`words_alpha.txt` 는 `.gitignore` 에 걸려 있어 저장소에 **없다**. 받아서 쓴다.
...
A·B 는 끝났다. 이어서 할 만한 것을 적어 둔다. **아무도 시키지 않았다 —
사람에게 먼저 물어본다.**

- **중복 예문 134건.** `npm run audit` 이 계속 경고한다. 서로 다른 두 단어가
  같은 문장을 쓰고 있다(`look` 과 `board` 가 "Look at the board, please."를
  같이 쓴다). 빈칸 문제에서 같은 문장이 두 번 나오면 아이가 문장을 외워
  버린다. 다만 고치려면 예문을 새로 써야 해서 작지 않다.
- **뜻 232건의 '확인 필요' 목록.** 전부 위키낱말사전 쪽이 성긴 자리로 보이지만
  사람 눈으로 한 번 더 훑을 수는 있다. `LIMIT=2000 npm run verify:meaning` 으로
  뽑는다.
- **실기기 확인.** 번들이 빌드되는 것까지만 봤다. 품사 칩과 유의어 소리가
  실제 화면에서 어떻게 보이는지는 아직 아무도 못 봤다.

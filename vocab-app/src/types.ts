...
export interface DailyRecord {
   ...
   * 오늘 **끝까지 푼 갈래들.**
   *
   * 예전에는 켠 갈래를 전부 한 줄로 이어 붙여 한 판에 다 풀게 했다. 영어와
   * 국어를 둘 다 켜면 한 번에 78문제가 되어 앉은자리에서 다 해야 했고,
   * 영어를 끝내고 쉬면 국어는 시작도 못 한 채 하루가 갔다.
   *
   * 지금은 갈래마다 따로 들어가 푼다. 그래서 "오늘 다 했는지" 도 갈래마다
   * 세어야 한다 — **켠 갈래가 전부 여기 들어와야 하루가 끝난다.**
   *
...
export interface RewardRequest {
  ...
  origin: RewardOrigin;
  /**
   * ── 아래 셋은 **다른 폰의 아이**가 보낸 신청에만 있다 ────────────────
   *
   * 아이가 제 폰을 쓰면 부모 폰에 그 아이 프로필이 없다. 그래서 `profileId`
   * 로는 누구 것인지 가릴 수가 없어 이름을 함께 적는다(`profileId` 에는
   * `remote:수빈` 처럼 넣어 이 폰의 아이와 안 섞이게 한다).
   */
  /** 다른 폰의 아이 이름. 이 폰에 프로필이 있는 아이면 없다. */
  childName?: string;
  /**
   * 그 아이 폰이 붙인 번호.
   *
   * 부모가 승인하면 이 번호를 되보내고, 아이 폰이 그것으로 제 신청을 찾아
   * 상태를 바꾼다. 이게 없으면 부모가 승인해도 아이 화면은 「기다리는 중」
   * 그대로다.
   */
  askId?: string;
  /** 판단 결과를 되보낼 주소 */
...
export interface ProfileSettings {
   ...
   * 하루에 새로 만날 **국어 어휘** 수.
   *
   * 영어와 따로 둔다. 국어는 한 낱말이 뜻마다 문항으로 갈려서, 같은 개수를
   * 잡아도 실제 문항 수가 영어보다 훨씬 많아진다. 6개로 못박아 두었더니
   * 한 판이 60문항 가까이 되어 "한 번에 너무 많다" 는 말을 들었다.
   *
   * **없으면 KO_PER_DAY(6)** 를 쓴다 — 이 칸이 생기기 전에 저장된 프로필에는
   * 들어 있지 않다.
   */
  koNewPerDay?: number;
  /**
...
export interface KnownChild {
   ...
   * ── 부모가 이 아이에게 **보낸** 설정 ────────────────────────
   *
   * 진짜 값은 아이 폰 안에 있다. 여기 있는 것은 **부모가 마지막으로 보낸
   * 것**이고, 화면에 무엇을 골라 두었는지 보여 주려고 적어 둔다. 안 적어
   * 두면 부모가 레벨을 바꿔 보낸 뒤 다시 들어왔을 때 화면이 아무것도
   * 안 골라진 상태로 돌아가, 보낸 것이 안 간 줄 알고 또 보내게 된다.
   *
   * 아이가 제 폰에서 스스로 바꾼 것까지는 알 수 없다. 그래서 화면에
   * 「보낸 값」 이라고 밝힌다 — 없는 것을 아는 척하지 않는다.
   */
  sentLevel?: LevelId;
  sentKoLevel?: LevelId;
  /** 이 아이에게 보낸 요청권 금액. 없으면 기기 기본값을 쓰는 중이다. */
  rates?: AwardRates;
  /**
   * 보낸 갈래와 하루 분량.
   *
   * "영어, 국어, 일상 문장 세 가지를 고를 수 있는데 이 선택은 어디서 하나요"
   * 라는 물음을 들었다. 그때까지 그 자리는 **아이 폰에만** 있었다 — 부모가
   * 아이 폰을 걷어 와야 갈래를 켜고 끌 수 있었던 셈이다.
   */
...
export interface ReceivedReport {
  ...
  receivedAt: number;
  /**
   * ── 아래 넷이 있어야 부모 폰에서 **달력과 날짜별 보고서**를 그릴 수 있다 ──
   *
   * 예전에는 `headline` 한 줄만 받았다. 그것은 사람이 읽는 글이라 숫자로
   * 되돌릴 수가 없어서, 다른 폰의 아이는 「마지막 소식」 한 줄이 전부였다.
   *
   * **없을 수 있다.** 아이 폰이 아직 옛 판이면 안 온다. 그런 날은 화면에서
   * 「아이 폰을 새로 깔면 자세히 볼 수 있어요」 라고 밝힌다 — 어림잡아 채우면
   * 아이가 받은 적 없는 정답률이 부모 화면에 숫자로 뜬다.
   */
  bySubject?: Partial<Record<Subject, SubjectTally>>;
  /** 그날 틀린 낱말 id. 이름과 뜻은 이 폰의 어휘에서 찾는다. */
  wrongIds?: string[];
  studied?: number;
  goal?: number;
}

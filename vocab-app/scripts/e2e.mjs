 ...
 * **부르는 자리가 여기 있어야 한다.**
 *
 * 아이 개별 설정 안에만 두었더니 ⚙️ 설정 → 아이들 기본 설정 → 아이 고르기
 * → 한참 아래로, 네 단계였다. "공부하자는 통보 버튼이 없어졌는데" 라는 말이
 * 그래서 나왔다 — 없앤 적은 없지만 그만큼 멀면 없는 것과 같다.
 *
 * 홈에서 한 번 눌러 닿는 이 화면에 있어야 한다.
 */
ok('여기서 바로 공부하세요를 보낼 수 있다', await has(page, '공부하세요', 3000));
